...
Isotope information is not saved in the zmat.
...
from zmats.exceptions import ZMatError
...
def xyz_to_zmat(xyz: dict,
    ...
    Args:
        ...
        mol (RMG Molecule, optional): The corresponding RMG Molecule. If given, the bonding information will be used
                                      to generate a more meaningful zmat.
        adj_list: The RMG adjacency list representation of the molecule, will be used to derive an RMG molecule object
                  if mol is ``None``.
        smiles: The SMILES representation of the molecule, will be used to derive an RMG molecule object
                if both mol and adjlist are ``None``.
    ...
    if mol is None and adj_list is not None:
        mol = Molecule().from_adjacency_list(adjlist=adj_list)
    ...
    if consolidate and not constraints:
        try:
            ...
            print(f'Could not consolidate zmat, got:\n{e.__class__}: {str(e)}')
            print(f'Generating zmat without consolidation.')
...
def determine_r_atoms(zmat: dict,
                      xyz: dict,
                      connectivity: dict,
                      n: int,
                      atom_index: int,
                      r_constraint: tuple = None,
        ...
        for atom_c in connectivity[atom_index]:
            ...
            while len(atom_list_to_explore1 + atom_list_to_explore2):
                ...
                if len(top) >= 2:
                    ...
                    angle = calculate_angle(coords=xyz['coords'], atoms=[atom_index] + top[-2:], units='degs')
...
def determine_a_atoms(zmat: dict,
                      coords: list or tuple,
                      connectivity: dict,
                      r_atoms: list,
                      n: int,
                      atom_index: int,
                      a_constraint: tuple = None,
                      a_constraint_type: str = None,
                      trivial_assignment: bool = False,
                      ) -> list:
    """
...
def determine_d_atoms(zmat: dict,
                      xyz: dict,
                      coords: list or tuple,
                      connectivity: dict,
                      a_atoms: list,
                      n: int,
                      atom_index: int,
                      d_constraint: tuple = None,
                      d_constraint_type: str = None,
                      specific_atom: int = None,
                      dummy: bool = False,
                      ) -> list:
    """
    ...
    if d_atoms is not None:
        ...
        if len(set(d_atoms)) != 4:
            print(f'Could not come up with four unique d_atoms (d_atoms = {d_atoms}). '
...
def _add_nth_atom_to_zmat(zmat: dict,
                          xyz: dict,
                          connectivity: dict,
                          n: int,
                          atom_index: int,
    ...
    if all([passed for passed in [r_constraints_passed, a_constraints_passed, d_constraints_passed]]):
        ...
        if (r_constraint_type == 'R_atom' or a_constraint_type == 'A_atom' or d_constraint_type == 'D_atom') \
            ...
            print(f'Skipping atom index {atom_index} when creating a zmat due to a specified _atom constraint.')
...
def zmat_to_coords(zmat: dict,
    ...
    Returns:
        tuple[list: The cartesian coordinates,
...
def check_atom_r_constraints(atom_index: int,
    ...
    Returns:
        tuple[tuple: The atom index to which the atom being checked is constrained. ``None`` if it is not constrained,
...
def check_atom_a_constraints(atom_index: int,
    ...
    Returns:
        tuple[tuple: The atom indices to which the atom being checked is constrained. ``None`` if it is not constrained,
...
def get_atom_order_from_mol(mol: Molecule,
    ...
    Returns:
        tuple[atom_order (list): The atom order, 0-indexed,
              connectivity (dict): Keys are atom indices, values are tuples of respective edges, ordered with heavy
                                   atoms first. All indices are 0-indexed, corresponding to atom indices in ``mol``
                                   (not in the zmat),
             ]
    """
    ...
    for constraint_type, constraint_list in constraints_dict.items():
        ...
        if constraint_type == 'D_group':
            for constraint_indices in constraint_list:
                ...
                # since the first atom (index 0) is the constrained one being changed,
                # this top is defined from the second atom in the torsion (index 1) to it

                # print(f'determining top_d for {constraint_indices[1]} -> {constraint_indices[0]}')  # debugging line
...
def get_atom_order_from_xyz(xyz: dict) -> tuple:
    """
    ...
    Returns:
        tuple[list: The atom order, 0-indexed,
...
def compare_zmats(zmat1: dict,
                  zmat2: dict,
                  r_tol: float = None,
                  a_tol: float = None,
                  d_tol: float = None,
                  verbose: bool = False,
                  symmetric_torsions: dict = None,
                  ) -> bool:
    """
    ...
    if zmat1['map'] != zmat2['map']:
        if verbose:
            print(f'zmats have different maps:\n{zmat1["map"]}\n{zmat2["map"]}')
        return False
    if len(zmat1['symbols']) != len(zmat2['symbols']):
        if verbose:
            print(f'zmats have different symbols:\n{zmat1["symbols"]}\n{zmat2["symbols"]}')
        return False
    for coord in zmat1['coords']:
        if coord not in zmat2['coords']:
            if verbose:
                print(f'zmats differ since the coordinates {coord} are missing from zmat2.')
            return False
        if coord is None:
            continue
        for var in coord:
            if var is not None:
                ...
                if abs(val1 - val2) > tol[key_type]:
                    ...
                    if sigma is None:
                        if verbose:
                            print(f'zmats differ since the respective values for var {var} in zmat1 and zmat2 '
...
def determine_top_group_indices(mol: Molecule,
                                atom1: Atom,
                                atom2: Atom,
                                ) -> list:
    """
...
def calculate_distance(coords: list or tuple,
    ...
    Args:
        ...
        atoms (list): The 2 atoms to calculate defining the vector for which the length will be calculated (0-indexed).
    ...
    if len(atoms) != 2:
        raise ZMatError(f'distance atom list must be of length two, got {len(atoms)}')
    if len(set(atoms)) < 2:
        raise ZMatError(f'some atoms are repetitive: {atoms}')
    new_atoms = list()
    for atom in atoms:
    ...
    if not all([isinstance(a, int) for a in new_atoms]):
        raise ZMatError(f'all entries in atoms must be integers, got: {new_atoms} ({[type(a) for a in new_atoms]})')
...
def calculate_angle(coords: list or tuple,
    ...
    Args:
        coords (list, tuple): The array-format or tuple-format coordinates.
        atoms (list): The 3 atoms defining the angle (0-indexed).
    ...
    if not isinstance(coords, (list, tuple)):
        raise TypeError(f'coords must be a list or a tuple, got\n{coords}\nwhich is a {type(coords)}')
    if len(atoms) != 3:
        raise ZMatError(f'angle atom list must be of length three, got {len(atoms)}')
    if len(set(atoms)) < 3:
        raise ZMatError(f'some atoms are repetitive: {atoms}')
    new_atoms = list()
    for atom in atoms:
...
def calculate_dihedral_angle(coords: list or tuple,
    ...
    Args:
        coords (list, tuple): The array-format or tuple-format coordinates.
        torsion (list): The 4 atoms defining the dihedral angle (0-indexed).
        units (str, optional): The desired units, either 'rads' for radians, or 'degs' for degrees.
    ...
    Raises:
        ZMatError: If ``index`` is out of range, or ``torsion`` is of wrong length or has repeating indices.
        TypeError: If ``coords`` is of wrong type.
    """
    if isinstance(coords, dict) and 'coords' in coords:
        coords = coords['coords']
    if not isinstance(coords, (list, tuple)):
        raise TypeError(f'coords must be a list or a tuple, got\n{coords}\nwhich is a {type(coords)}')
    if len(torsion) != 4:
        raise ZMatError(f'torsion atom list must be of length four, got {len(torsion)}')
    if len(set(torsion)) < 4:
        raise ZMatError(f'some indices in torsion are repetitive: {torsion}')
    new_torsion = list()
    for atom in torsion:
        if isinstance(atom, str) and 'X' in atom:
    ...
    if not all([isinstance(t, int) for t in new_torsion]):
        raise ZMatError(f'all entries in torsion must be integers, got: {new_torsion} '
...
def get_angle(v1: list,
    ...
    """
    if len(v1) != len(v2):
        raise ZMatError(f'v1 and v2 must be the same length, got {len(v1)} and {len(v2)}.')
...
def get_dihedral(v1: list,
    ...
    if len(v1) != 3 or len(v2) != 3 or len(v3) != 3:
        raise ZMatError(f'v1, v2, and v3 must have a length of three, got {len(v1)}, {len(v2)}, and {len(v3)}.')
    ...
    if np.isnan(dihedral):
        raise ZMatError('Could not calculate a dihedral angle')
    conversion = 180 / math.pi if 'degs' in units else 1
